...
<div class="col-lg-9">
    <div class="panel panel-info">
        ...
        <table id="detail-grid" class="table table-striped">
            ...
            function renderRow($model, $index, $prices)
            {
                ob_start();
                        ...
                        <ul class="nav nav-list">
                            ...
                            <li>
                                <div style="display: none">
                                    <?php
                                    foreach ($prices as $price_ct) {
                                        echo Html::activeHiddenInput($model, "[$index]salesPrices[{$price_ct['id_price_category']}]", [
                                            'id' => false,
                                            'data' => [
                                                'field' => 'sales_price',
                                                'price_ct' => $price_ct['id_price_category']
                                            ]
                                        ]) . "\n";
                                    }
                                    ?>
            ...
            foreach ($details as $index => $model) {
                $rows[] = renderRow($model, $index, $prices);
            }
            echo Html::tag('tbody', implode("\n", $rows), ['data-template' => renderRow(new PurchaseDtl, '_index_', $prices)])
            ?>
        </table>
    </div>
</div>